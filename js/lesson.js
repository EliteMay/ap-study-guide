(() => {
  ...
  function renderProgressSummary(lessonId, total, currentAnswered = 0, currentCorrect = 0) {
    ...
    root.innerHTML = `${current}<span>Best ${best} / ${total}</span><span class="${state.state === 'mastered' ? 'is-complete' : ''}">${escapeHtml(state.label)}</span>`;
  }
  ...
  function renderHero(entry, lesson) {
    ...
    $('lesson-hero').innerHTML = `<p class="eyebrow">${escapeHtml(lesson.meta.id)} / STRUCTURED LESSON</p><h1>${escapeHtml(lesson.meta.title)}</h1><p class="lead">${escapeHtml(lesson.intro || '')}</p><div class="lesson-meta-row"><span>IPA中分類 ${(lesson.meta.officialMiddleCodes || []).map(escapeHtml).join(', ')}</span><span>${escapeHtml(entry.status || lesson.meta.qualityStatus || '')}</span>${lesson.meta.estimatedMinutes ? `<span>目安 ${escapeHtml(lesson.meta.estimatedMinutes)}分</span>` : ''}</div>`;
  }

  function renderObjectives(lesson) {
    const objectives = lesson.objectives || [];
    $('lesson-objectives').innerHTML = objectives.length ? `<ol>${objectives.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ol>` : '<p>学習目標は未設定です。</p>';
  }
  ...
  function renderDiagram(section) {
    return (section.diagrams || []).map(diagram => {
      ...
      const nodeHtml = nodes.map((node, index) => `<div class="structure-item" role="listitem"><div class="structure-node"><span class="structure-node-title">${escapeHtml(node.title || '')}</span><strong>${escapeHtml(node.value || '')}</strong>${node.meta ? `<small>${escapeHtml(node.meta)}</small>` : ''}</div>${connector && index < nodes.length - 1 ? `<span class="structure-connector" aria-hidden="true">${escapeHtml(connector)}</span>` : ''}</div>`).join('');
  ...
  function renderSection(section, index) {
    ...
    if (section.type === 'text') return `<section class="lesson-block"><h2>${title}</h2>${(section.paragraphs || []).map(p => `<p>${escapeHtml(p)}</p>`).join('')}</section>`;
    if (section.type === 'comparison') return `<section class="lesson-block"><h2>${title}</h2>${table(section.columns, section.rows, 'comparison-table')}${section.note ? `<div class="lesson-note"><strong>ポイント</strong><p>${escapeHtml(section.note)}</p></div>` : ''}</section>`;
    if (section.type === 'diagram') return `<section class="lesson-block diagram-block"><h2>${title}</h2>${renderDiagram(section)}</section>`;
    if (section.type === 'code-trace') return `<section class="lesson-block code-trace-block"><h2>${title}</h2>${section.question ? `<p class="lesson-question-lead">${escapeHtml(section.question)}</p>` : ''}<pre class="lesson-code"><code>${escapeHtml((section.code || []).join('\n'))}</code></pre><h3>実行トレース</h3>${table(section.traceColumns, section.traceRows, 'trace-table')}<div class="lesson-answer"><strong>答え</strong><p>${escapeHtml(section.answer || '')}</p></div>${section.explanation ? `<p>${escapeHtml(section.explanation)}</p>` : ''}</section>`;
    if (section.type === 'worked-example') {
      ...
      const result = Array.isArray(section.resultColumns) && Array.isArray(section.resultRows) ? `<h3>${escapeHtml(section.resultTitle || '結果')}</h3>${table(section.resultColumns, section.resultRows, 'worked-result-table')}` : '';
    ...
    if (section.type === 'steps') return `<section class="lesson-block"><h2>${title}</h2><div class="lesson-step-list">${(section.items || []).map(item => `<article><strong>${escapeHtml(item.label)}</strong><p>${escapeHtml(item.body)}</p></article>`).join('')}</div></section>`;
    if (section.type === 'mistakes') return `<section class="lesson-block mistake-block"><h2>${title}</h2><ul class="mistake-list">${(section.items || []).map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul></section>`;
  ...
  function renderChecks(lesson) {
    ...
    $('lesson-questions').innerHTML = checks.map((q, qi) => `<article class="check-question" data-question="${qi}"><h3>Q${qi + 1}. ${escapeHtml(q.prompt)}</h3><div class="check-options">${(q.options || []).map((option, oi) => `<button type="button" data-option="${oi}">${escapeHtml(option)}</button>`).join('')}</div><div class="check-feedback" hidden></div></article>`).join('');
  ...
  function renderLessonNav(entry, lesson, lessons) {
    ...
    const rows = [];
    if (previous) rows.push({ id:previous.id, label:`前の教材：${previous.title}`, available:true, direction:'PREV' });
    if (explicitNext.length) {
      for (const item of explicitNext) {
        const resolved = indexedById.get(item.lessonId);
        rows.push({ id:item.lessonId, label:resolved?.title || item.label || '', available:Boolean(resolved), direction:'NEXT' });
      }
    } else if (indexNext) rows.push({ id:indexNext.id, label:indexNext.title, available:true, direction:'NEXT' });
  ...
  async function init() {
    try {
      ...
      $('lesson-status').innerHTML = `<strong>${escapeHtml(lesson.meta.id)}</strong> は構造化教材です。確認問題は全問回答だけでは完了にならず、75%以上で理解確認済みになります。14日後は再確認対象です。`;

...
for (const required of ['home-quick-search','学習メニュー','今日やることが、','html/glossary.html','単語を調べる','css/home-launch.css','home-hero-grid','home-command-panel']) if (!home.includes(required)) fail(`homepage missing ${required}`);
...
console.log(`[glossary] OK: ${total} legacy terms unified, ${60}-result pagination, lazy rich details, narrow-layout guards, refreshed home entry, no static count duplication.`);

...
const practiceDeclared = (practiceManifest.files || []).reduce((sum,item) => sum + Number(item.count || 0), 0);
...
const caseDeclared = (caseManifest.files || []).reduce((sum,item) => sum + Number(item.count || 0), 0);
const curriculum = readJson('json/curriculum/ap-2026-map.json');
if (!lessons.length || questions.length !== practiceDeclared || questions.length !== Number(practiceManifest.meta?.questionCount || 0) || cases.length !== caseDeclared || cases.length !== Number(caseManifest.meta?.caseCount || 0)) fail('core manifest counts mismatch');
...
console.log(`[progress] OK: current mastery dashboard connects ${lessons.length} lessons + ${questions.length} short questions + ${cases.length} long cases across ${curriculum.studyUnits.length} units with complete loader error states.`);